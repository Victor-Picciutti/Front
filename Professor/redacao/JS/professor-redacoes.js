...
async function carregarRedacoes() {
    ...
    try {
        ...
        // Filtrar redações que NÃO são do professor (aluno diferente de 6)
        const redacoesAlunos = todasRedacoes.filter(red => red.aluno?.id !== 6);
        ...
        return `
            <div class="redacao-card" onclick="abrirModalCorrecao(${red.idRedacao})">
                ...
                <div class="redacao-preview">
                    ${escapeHtml((red.textoRedacao || '').substring(0, 120))}...
                </div>
...
function filtrarRedacoes() {
    ...
    let filtradas = todasRedacoes.filter(red => red.aluno?.id !== 6);
...
function inicializarEventos() {
    // Botão calcular nota por competências
